...
class TapGuideViewModelTest {

    private val dispatcher = StandardTestDispatcher()
    private val settingsFlow = MutableStateFlow(TapSenseSettings(reduceMotion = true))
    ...
    private fun viewModel(): TapGuideViewModel {
        val vm = TapGuideViewModel(resolveAntennaLocationUseCase, activeDeviceSignalsProvider, settingsRepository, nfcStateObserver, logger)
        dispatcher.scheduler.advanceUntilIdle()
        return vm
    }

    @Test
    fun `resolves the current device once on init`() = runTest(dispatcher) {
        ...
        assertThat(vm.uiState.value.reduceMotion).isTrue()
    }
