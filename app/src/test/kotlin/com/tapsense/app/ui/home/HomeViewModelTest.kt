...
class HomeViewModelTest {
    ...
    private fun viewModel(): HomeViewModel {
        val vm = HomeViewModel(resolveAntennaLocationUseCase, activeDeviceSignalsProvider, settingsRepository, nfcStateObserver, logger)
    ...
    fun `resolves the current device and exposes a friendly display name`() = runTest(dispatcher) {
        ...
        assertThat(vm.uiState.value.displayModel).isEqualTo("Google Pixel 8")
        assertThat(vm.uiState.value.displayManufacturer).isEqualTo("Google")
    ...
    @Test
    fun `reflects live NFC on-off state`() = runTest(dispatcher) {
        coEvery { resolveAntennaLocationUseCase(any()) } returns fakeProfile
        val vm = viewModel()
        ...
        assertThat(vm.uiState.value.isNfcOn).isTrue()
    }

    @Test
    fun `refresh re-runs resolution`() = runTest(dispatcher) {
        coEvery { resolveAntennaLocationUseCase(any()) } returns fakeProfile
        val vm = viewModel()
